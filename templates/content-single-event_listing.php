                    ...
                    <div class="wpem-row">
                        <div class="wpem-col-xs-12 wpem-col-sm-7 wpem-col-md-8 wpem-single-event-left-content">
                            ...
                            <div class="wpem-single-event-body-content">
                                <section class="usersWidget">
                                <?php
                                function getIdOfEventMembers($eventId) {
                                    global $wpdb;
                                    return $wpdb->get_results("SELECT display_name, ID FROM {$wpdb->prefix}event_members INNER JOIN {$wpdb->prefix}users ON {$wpdb->prefix}users.ID = {$wpdb->prefix}event_members.EventMember AND EventId = {$eventId} ORDER BY dateOfRegister DESC");
                                }

                                $eventId = get_the_ID();
                                $eventMembers = getIdOfEventMembers($eventId);
                                $eventMembersNumber = count($eventMembers);
                                ?>
                                <h3>Goście <?php echo $eventMembers ? "(${eventMembersNumber})" : null; ?></h3>
                                <ul class="users">
                                    <?php foreach (getIdOfEventMembers($eventId) as $row => $obj) : ?>
                                        <?php $currentUser = get_userdata($obj->ID); ?>
                                        <li class="users__item">
                                            <a class="users__link" href="<?php echo get_author_posts_url($obj->ID); ?>">
                                                <?php echo get_avatar($obj->ID, 48); ?>
                                                <span class="users__linkText">
                                                    <?php if ($currentUser->first_name || $currentUser->last_name) {
                                                        echo $currentUser->first_name . " " . $currentUser->last_name;
                                                    } else {
